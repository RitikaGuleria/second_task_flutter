...
class _FoodListState extends ConsumerState<FoodList> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text("Food Category List"),),
      body: Consumer(
          builder: (context, ref, child) {
            AsyncValue<List<CategoriesNames>> cl = ref.watch(
                aPIService_ProviderProvider);
            return cl.when(data: (categoryData) {
              return ListView.builder(itemCount: categoryData.length,
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        leading: ClipRRect(borderRadius: BorderRadius.circular(
                            22),
                          child: Image.network(
                            item.strCategoryThumb, fit: BoxFit.fill,
                            height: 45,
                            width: 45,)
                          ,),
                          ...
                          child: Row(
                            children: [
                              IconButton(onPressed: () {
                                ref.read(counter_ProviderProvider.notifier)
                                    .decrement();
